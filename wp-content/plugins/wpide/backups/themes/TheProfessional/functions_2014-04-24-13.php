<?php /* start WPide restore code */
                                    if ($_POST["restorewpnonce"] === "63d680bd76d4ff9b4947e178222f2e25faa048d5d1"){
                                        if ( file_put_contents ( "/data/wwwroot/smilepoint.hu/http/wp-content/themes/TheProfessional/functions.php" ,  preg_replace("#<\?php /\* start WPide(.*)end WPide restore code \*/ \?>#s", "", file_get_contents("/data/wwwroot/smilepoint.hu/http/wp-content/plugins/wpide/backups/themes/TheProfessional/functions_2014-04-24-13.php") )  ) ){
...
function bgmpShortcodeCalled()
{
    if(true) add_filter( 'bgmp_map-shortcode-called', '__return_true' );
}
add_action( 'wp', 'bgmpShortcodeCalled' );

/*
add_action('init', 'spCreateCategoryTaxonomy'));

function spCreateCategoryTaxonomy()
{
	if (did_action( 'init' ) !== 1 ) return;
...
add_action('save_post', 'spSaveCityTaxonomy', 30, 1);

function spSaveCityTaxonomy($postID) {
	...
	// Check preconditions
	if (isset($_GET[ 'action' ]) && in_array($_GET[ 'action' ], $ignoredActions)) return;
	if (!$post || $post->post_type != 'bgmp' || !current_user_can('edit_posts')) return;
		
	if ((defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) || $post->post_status == 'auto-draft') return;

	$city = get_post_meta($post->ID, BasicGoogleMapsPlacemarks::PREFIX . 'city', true);

	if ($city != '') {
		$term = term_exists($city, BasicGoogleMapsPlacemarks::TAXONOMY);
		if (!is_array($term)) $term = wp_insert_term($city, BasicGoogleMapsPlacemarks::TAXONOMY);
		wp_set_post_terms($post->ID, array(intval($term['term_id'])), BasicGoogleMapsPlacemarks::TAXONOMY);
	} else {
		wp_delete_object_term_relationships($post->ID, BasicGoogleMapsPlacemarks::TAXONOMY);
	}
}

//Define the function
 
function customize_cities_archive_display($query) {
    if (($query->is_main_query()) && (is_tax('bgmp-category')))
...
add_action('pre_get_posts', 'customize_cities_archive_display');
