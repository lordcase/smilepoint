...
if( !class_exists( 'BasicGoogleMapsPlacemarks' ) )
{
	...
	class BasicGoogleMapsPlacemarks
	{
		...
		protected function cleanMapShortcodeArguments( $arguments )
		{
			...
			{
				if (is_numeric($arguments['categories'])) {
					$arguments['categories'] = array(intval($arguments['categories']));
				}
				else if (is_string($arguments['categories'])) {
		...
		public function loadResources()
		{
			...
			wp_register_script(
				'googleMapsAPI',
				'http'. ( is_ssl() ? 's' : '' ) .'://maps.google.com/maps/api/js?sensor=false&libraries=places' . $googleMapsLanguage,
				array(),
				false,
				true
			);
		...
		public function createPostType()
		{
			...
			{
				$labels = array(
					'name'					=> __( 'Szalonok', 'bgmp' ),
					'singular_name'			=> __( 'Szalon', 'bgmp' ),
					'add_new'				=> __( 'Új hozzáadása', 'bgmp' ),
					'add_new_item'			=> __( 'Új szalon hozzáadása', 'bgmp' ),
					'edit'					=> __( 'Szerkesztés', 'bgmp' ),
					'edit_item'				=> __( 'Szalon szerkesztése', 'bgmp' ),
					'new_item'				=> __( 'Új szalon', 'bgmp' ),
					'view'					=> __( 'Szalon megtekintése', 'bgmp' ),
					'view_item'				=> __( 'View Placemark', 'bgmp' ),
					'search_items'			=> __( 'Szalonok keresése', 'bgmp' ),
				...
				$postTypeParams = array(
					'labels'				=> $labels,
					'singular_label'		=> __( 'Szalonjaink', 'bgmp' ),
					'public'				=> true,
					'menu_position'			=> 20,
					'hierarchical'			=> false,
					'capability_type'		=> 'post',
					'rewrite'				=> array( 'slug' => 'szalonjaink', 'with_front' => true ),
					'query_var'				=> true,
					'supports'				=> array( 'custom-fields', 'title', 'editor', 'author', 'thumbnail', 'comments', 'revisions' )
				);
		...
		public function createCategoryTaxonomy()
		{
			...
			if( !taxonomy_exists( self::TAXONOMY ) )
			{
				$taxonomyParams = array(
					'label'					=> __( 'Város', 'bgmp' ),
					'labels'				=> array( 'name' => __( 'Városok', 'bgmp' ), 'singular_name' => __( 'Város', 'bgmp' ) ),
					'hierarchical'			=> false,
					'rewrite'				=> array('slug' => 'varosok'),
...
// DAHERO
			if( $coordinates )
			{
    			$geocodeResponse = wp_remote_get( 'http://maps.googleapis.com/maps/api/geocode/json?latlng='. $coordinates[ 'latitude' ] .','. $coordinates[ 'longitude' ] .'&sensor=false' );
    			...
    			if (!is_wp_error( $geocodeResponse ) && !empty($address->results)) {
				    ...
				    update_post_meta( $post->ID, self::PREFIX . 'city', $city );
    			} else {
    				update_post_meta( $post->ID, self::PREFIX . 'city', '' );
    			}
		...
		public function mapShortcode($attributes)
		{
			if (!wp_script_is('googleMapsAPI', 'queue') ||
				!wp_script_is('bgmp', 'queue') ||
				!wp_style_is(self::PREFIX.'style', 'queue'))
			{
				$error = sprintf(
			...
			$attributes = apply_filters(self::PREFIX . 'map-shortcode-arguments', $attributes);
			// @todo - deprecated b/c 1.9 output bgmpdata in post...
			$attributes = $this->cleanMapShortcodeArguments($attributes);
			
			ob_start();
			do_action(self::PREFIX . 'meta-address-before');
			require_once(dirname(__FILE__) . '/views/shortcode-bgmp-map.php');
			do_action(self::PREFIX . 'shortcode-bgmp-map-after');
			$output = ob_get_clean();
		...
		public function getMapPlacemarks($attributes)
		{
			...
			$query = apply_filters(self::PREFIX . 'get-placemarks-query', $query);
			// @todo - filter name deprecated
			...
			if ($publishedPlacemarks) {
				foreach ($publishedPlacemarks as $pp) {
					...
					$placemark = array(
						...
						'city'		=> array(
							'name' => (is_array($terms) && count($terms) > 0) ? $terms[0]->name : '',
							'slug' => (is_array($terms) && count($terms) > 0) ? $terms[0]->slug : '',
						),
						'latitude'		=> get_post_meta( $postID, self::PREFIX . 'latitude', true ),
						'longitude'		=> get_post_meta( $postID, self::PREFIX . 'longitude', true ),
						'details'		=> $pp->post_content ,		// note: don't use setup_postdata/get_the_content() in 
